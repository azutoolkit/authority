...
module Authority
  class AdminUserService
    ...
    def self.list(options : ListOptions = ListOptions.new) : Array(User)
      ...
      results = apply_status_filter(results, options.status)
    ...
    private def self.build_sorted_query(options : ListOptions)
      ...
      when "username"      then query.order(username: sort_direction)
      when "email"         then query.order(email: sort_direction)
      when "last_login_at" then query.order(last_login_at: sort_direction)
      else                      query.order(created_at: sort_direction)
      end
    end

    private def self.apply_search_filter(results : Array(User), search : String?) : Array(User)
      return results if search.nil? || search.empty?
      pattern = search.downcase
      results.select do |user|
    ...
    private def self.apply_status_filter(results : Array(User), status : String?) : Array(User)
      case status
      when "active" then results.select { |user| user.locked_at.nil? }
      when "locked" then results.select { |user| !user.locked_at.nil? }
      else               results
      end
    end
    ...
    def self.create(
      ...
      validation_error = validate_user_create(username, email, password, skip_password_validation)
      return validation_error if validation_error

      user = build_user(username, email, password, first_name, last_name, role, scope, email_verified)
      user.save!

      log_user_create_audit(user, actor, ip_address)
    ...
    # Validate user creation constraints
    private def self.validate_user_create(username : String, email : String, password : String, skip_password_validation : Bool) : Result?
    ...
    # Build a new user with initial values
    private def self.build_user(
      username : String,
      email : String,
      password : String,
      first_name : String,
    ...
    private def self.log_user_create_audit(user : User, actor : User?, ip_address : String?) : Nil
      return unless actor
    ...
    def self.update(
      ...
      validation_error = validate_user_update(user, username, email)
      return validation_error if validation_error

      old_values = capture_user_values(user)

      has_changes = apply_user_updates(user, username, email, first_name, last_name, role, scope, email_verified)
      return Result.new(success: true, user: user) unless has_changes

      user.updated_at = Time.utc
      user.update!

      log_user_update_audit(user, old_values, actor, ip_address)
    ...
    private def self.validate_user_update(user : User, username : String?, email : String?) : Result?
    ...
    # Capture current user values for audit diff
    private def self.capture_user_values(user : User) : Hash(String, String?)
      {
    ...
    # Apply updates to user fields, returns true if any changes were made
    private def self.apply_user_updates(
      user : User,
      username : String?,
      email : String?,
      first_name : String?,
    ...
    private def self.log_user_update_audit(user : User, old_values : Hash(String, String?), actor : User?, ip_address : String?) : Nil
      return unless actor

      new_values = capture_user_values(user)
